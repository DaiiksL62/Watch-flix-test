...
if(!empty($_POST)){
    ...
    $req = $pdo->prepare("INSERT INTO inscription SET username = ?, nom = ?, prenom = ?, email= ? , user_pass = ?, user_kind = ? , photo = ?");
    $password = password_hash($_POST['user_pass'], PASSWORD_BCRYPT);
    $req->execute([$_POST['username'],$_POST['nom'], $_POST['prenom'], $_POST['email'], $password, $_POST['user_kind'] ,$_FILES['photo'] = $avatar]);
